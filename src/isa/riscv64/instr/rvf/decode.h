...
static int table_op_zfbf(Decode *s);
#endif
...
def_THelper(op_fp) {
  ...
  #ifdef CONFIG_RV_ZFBF_MIN
    /*
     * Zfbfmin (fcvt.bf16.s / fcvt.s.bf16) overlaps the broad Zfh row "????? 10
     * ..." in encoding. Match op_zfbf first; otherwise decoding falls through
     * to op_zfh, which has no entry -> EXEC_ID_inv.
     */
    def_INSTR_TAB("01000 ?? 01000 ????? ??? ????? ????? ??", op_zfbf);
    def_INSTR_TAB("01000 ?? 00110 ????? ??? ????? ????? ??", op_zfbf);
  #endif
  #ifdef CONFIG_RV_ZFH_MIN
